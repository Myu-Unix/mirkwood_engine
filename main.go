...
func init() {
	...
	npc[0] = enemy{name: "Ghaz", race: "Level 1 Goblin", posx: 1200, posy: 700, hp_max: "8 HP", ac_armor_class: "AC 5", item1: "Club (1d4)", alive: 1}
	npc[1] = enemy{name: "Dhurg", race: "Level 2 Goblin Warg Rider", posx: 1100, posy: 750, hp_max: "10 HP", ac_armor_class: "AC 7", item1: "Hand-Axe (1d6)", alive: 1}
	npc[2] = enemy{name: "Dorg", race: "Level 1 Skeleton Archer", posx: 1150, posy: 800, hp_max: "5 HP", ac_armor_class: "AC 6", item1: "Longbow (1d6)", alive: 1}
	...
	if STATE_SHOW_SPLASH > 0 { // This shows the splashscreen
		...
		if STATE_LINK == 1 {
			if npc[STATE_ENEMY_SELECTED-1].alive == 1 {
				ebitenutil.DrawLine(screen, player[STATE_PLAYER_SELECTED-1].posx+16, player[STATE_PLAYER_SELECTED-1].posy+32, npc[STATE_ENEMY_SELECTED-1].posx+16, npc[STATE_ENEMY_SELECTED-1].posy+32, color.RGBA{255, 128, 0, 255})
				...
				text.Draw(screen, string(strconv.Itoa(int(distance))), g.assets.fonts.mplusSmallFont, int(distance*5+player[STATE_PLAYER_SELECTED-1].posx), int(distance*5+player[STATE_PLAYER_SELECTED-1].posy), color.White)
				text.Draw(screen, "ft", g.assets.fonts.mplusSmallFont, int(distance*5+player[STATE_PLAYER_SELECTED-1].posx+30), int(distance*5+player[STATE_PLAYER_SELECTED-1].posy), color.White)
			}
		}
		...
		// Player "token" data
		text.Draw(screen, string(player[STATE_PLAYER_SELECTED-1].name), g.assets.fonts.mplusSmallFont, int(player[STATE_PLAYER_SELECTED-1].posx+48), int(player[STATE_PLAYER_SELECTED-1].posy), color.White)
		// TEST - JSON gathered
		...
		text.Draw(screen, string(player[STATE_PLAYER_SELECTED-1].hp_max), g.assets.fonts.mplusMiniFont, int(player[STATE_PLAYER_SELECTED-1].posx+64), int(player[STATE_PLAYER_SELECTED-1].posy+18), color.White)
		text.Draw(screen, string(player[STATE_PLAYER_SELECTED-1].ac_armor_class), g.assets.fonts.mplusMiniFont, int(player[STATE_PLAYER_SELECTED-1].posx+72), int(player[STATE_PLAYER_SELECTED-1].posy+32), color.White)
		text.Draw(screen, string(player[STATE_PLAYER_SELECTED-1].item1), g.assets.fonts.mplusMiniFont, int(player[STATE_PLAYER_SELECTED-1].posx+72), int(player[STATE_PLAYER_SELECTED-1].posy+46), color.White)
		...
		if STATE_SHOW_DEBUG == 1 {
			...
			text.Draw(screen, strconv.Itoa(STATE_PLAYER_SELECTED), g.assets.fonts.mplusSmallFont, 156, 560, color.White)
			text.Draw(screen, strconv.Itoa(STATE_ENEMY_SELECTED), g.assets.fonts.mplusSmallFont, 156, 600, color.White)
			text.Draw(screen, strconv.Itoa(STATE_ROUND), g.assets.fonts.mplusSmallFont, 156, 640, color.White)
		}
		...
		if STATE_SHOW_INVENTORY == 1 {
			...
			text.Draw(screen, string(player[STATE_PLAYER_SELECTED-1].name), g.assets.fonts.mplusNormalFont, 1480, 82, color.White)
			text.Draw(screen, string(player[STATE_PLAYER_SELECTED-1].class), g.assets.fonts.mplusSmallFont, 1480, 114, color.White)
			text.Draw(screen, string(player[STATE_PLAYER_SELECTED-1].hp_max), g.assets.fonts.mplusSmallFont, 1480, 146, color.White)
			text.Draw(screen, string(player[STATE_PLAYER_SELECTED-1].ac_armor_class), g.assets.fonts.mplusSmallFont, 1540, 146, color.White)
			text.Draw(screen, string(player[STATE_PLAYER_SELECTED-1].alignment), g.assets.fonts.mplusSmallFont, 1490, 178, color.White)
			...
			text.Draw(screen, string(player[STATE_PLAYER_SELECTED-1].STR), g.assets.fonts.mplusMiniFont, 1770, 70, color.White)
			text.Draw(screen, string(player[STATE_PLAYER_SELECTED-1].DEX), g.assets.fonts.mplusMiniFont, 1770, 90, color.White)
			text.Draw(screen, string(player[STATE_PLAYER_SELECTED-1].CON), g.assets.fonts.mplusMiniFont, 1770, 110, color.White)
			text.Draw(screen, string(player[STATE_PLAYER_SELECTED-1].INT), g.assets.fonts.mplusMiniFont, 1770, 130, color.White)
			text.Draw(screen, string(player[STATE_PLAYER_SELECTED-1].WIS), g.assets.fonts.mplusMiniFont, 1770, 150, color.White)
			text.Draw(screen, string(player[STATE_PLAYER_SELECTED-1].CHA), g.assets.fonts.mplusMiniFont, 1770, 170, color.White)
			text.Draw(screen, string(player[STATE_PLAYER_SELECTED-1].item1), g.assets.fonts.mplusSmallFont, 1532, 270, color.White)
			text.Draw(screen, string(player[STATE_PLAYER_SELECTED-1].item2), g.assets.fonts.mplusSmallFont, 1532, 310, color.White)
			text.Draw(screen, string(player[STATE_PLAYER_SELECTED-1].item3), g.assets.fonts.mplusSmallFont, 1532, 350, color.White)
			text.Draw(screen, string(player[STATE_PLAYER_SELECTED-1].item4), g.assets.fonts.mplusSmallFont, 1532, 390, color.White)
			text.Draw(screen, string(player[STATE_PLAYER_SELECTED-1].item5), g.assets.fonts.mplusSmallFont, 1532, 430, color.White)
			//text.Draw(screen, string(player[STATE_PLAYER_SELECTED-1].item6), mplusSmallFont, 1532, 470, color.White)
		} // INVENTORY CARD END

		// Show/hide enemy data
		if npc[STATE_ENEMY_SELECTED-1].alive == 1 {
			text.Draw(screen, string(npc[STATE_ENEMY_SELECTED-1].race), g.assets.fonts.mplusSmallFont, int(npc[STATE_ENEMY_SELECTED-1].posx+48), int(npc[STATE_ENEMY_SELECTED-1].posy-10), color.White)
			text.Draw(screen, string(npc[STATE_ENEMY_SELECTED-1].hp_max), g.assets.fonts.mplusMiniFont, int(npc[STATE_ENEMY_SELECTED-1].posx+64), int(npc[STATE_ENEMY_SELECTED-1].posy+18), color.White)
			text.Draw(screen, string(npc[STATE_ENEMY_SELECTED-1].ac_armor_class), g.assets.fonts.mplusMiniFont, int(npc[STATE_ENEMY_SELECTED-1].posx+72), int(npc[STATE_ENEMY_SELECTED-1].posy+32), color.White)
			text.Draw(screen, string(npc[STATE_ENEMY_SELECTED-1].item1), g.assets.fonts.mplusMiniFont, int(npc[STATE_ENEMY_SELECTED-1].posx+72), int(npc[STATE_ENEMY_SELECTED-1].posy+46), color.White)
		}
